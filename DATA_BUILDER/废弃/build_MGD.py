"""构建「主视角视频 + 报告测量值」的 SFT 对话数据。

优化版本：一次遍历报告，同时构建所有诊断项的训练/测试数据。

对每条报告，仅选取主视角（primary_views）视频，并在诊断提示词中追加报告的实际
测量值（如心腔各径线数值），作为模型判断的辅助信息。
"""
import argparse
import os
...
def build_all_diag_data_once(save_path, reports, sample_num, diag_items, is_train=True):
    ...
    for diag_item in diag_items:
        ...
        primary_views = set(diag_info[diag_item]["primary_views"])
        ...
        diag_configs[diag_item] = {
            ...
            "P": P,
            "N": N,
            "primary_views": primary_views  # 转为set加速查找
        }
    ...
    for report in tqdm(reports, desc="处理报告"):
        ...
        for diag_item in diag_items:
            ...
            # 在提问中加入测量值，例如：「以及给出的测量值{...},请判断...」
            newprompt = config["prompt"].replace("判断", f"以及给出的测量值{mv},请判断")
