"""构建「多视角视频 + 默认提示词」的 SFT 对话数据。

优化版本：一次遍历报告，同时构建所有诊断项的训练/测试数据。

对每条报告，选取属于该诊断项 primary_views 或 supplementary_views 的全部视频，
一条报告生成一条包含多个视频的对话样本（human 提问带多个 <video> 占位符）。
"""
import argparse
import os
...
def build_all_diag_data_once(save_path, reports, sample_num, diag_items, is_train=True):
    ...
    for diag_item in diag_items:
        ...
        # 主视角 + 补充视角
        all_views = set(diag_info[diag_item]["primary_views"]) | set(diag_info[diag_item]["supplementary_views"])
    ...
    for report in tqdm(reports, desc="处理报告"):
        ...
        for diag_item in diag_items:
            ...
            all_views = config["all_views"]

            # 收集主视角与补充视角的所有视频
            multi_vs = []
            for view in view_to_video:
                if view in all_views:
                    multi_vs.append(view_to_video[view])
