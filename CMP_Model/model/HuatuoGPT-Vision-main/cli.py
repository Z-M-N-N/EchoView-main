...
class HuatuoChatbot():
    ...
    def inference(self, text, images=None, return_probs=False):
        '''
        text: str
        images: list[str]
        return_probs: bool, 是否返回 Yes/No 的概率
        '''
        ...
        with torch.inference_mode():
            if return_probs:
                # 使用 return_dict_in_generate=True 和 output_scores=True 获取 logits
                output_ids = self.model.generate(
                    input_ids,
                    images=image_tensors,
                    use_cache=True,
                ...
                # 获取第一个生成的 token 的 logits（即 Yes/No 的 logits）
                # output_ids.scores 是一个 tuple，每个元素是 [batch_size, vocab_size]
                if output_ids.scores is not None and len(output_ids.scores) > 0:
                    first_token_logits = output_ids.scores[0]  # 第一个生成 token 的 logits
                    probs = torch.softmax(first_token_logits, dim=-1)  # [batch_size, vocab_size]
                ...
                return answers, yes_prob_normalized, no_prob_normalized
