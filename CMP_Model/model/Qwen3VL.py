...
# ============ 主函数 ============
def main():
    parser = argparse.ArgumentParser(description='Qwen3-VL 视频理解推理脚本')
    parser.add_argument('--model_path', type=str, 
                       default="/home/mzhao/ECHO_VIEW/对比模型/Qwen3-VL-8B-Instruct", 
                       help='基础模型路径')
    parser.add_argument('--processor_path', type=str, 
                       default="/home/mzhao/ECHO_VIEW/对比模型/Qwen3-VL-8B-Instruct", 
                       help='处理器路径')
    parser.add_argument('--lora_path', type=str, default="", help='LoRA权重路径')
    parser.add_argument('--diag_item_config', type=str, 
                       default="/home/mzhao/ECHO_VIEW/EchoView-main/DATA_BUILDER/diag_info.json", 
                       help='诊断项配置文件')
    parser.add_argument('--device_map', type=str, default="auto", 
                       help='设备映射策略 (auto/cuda/cpu)')
    parser.add_argument('--torch_dtype', type=str, default="auto", 
                       choices=['auto', 'float16', 'float32', 'bfloat16'], 
                       help='torch数据类型')
    parser.add_argument('--cuda_num', type=int, default=2, 
                       help='指定使用的GPU编号 (0-7)')
    parser.add_argument('--expandable_segments', type=bool, default=True, 
                       help='是否启用PyTorch动态显存扩展')
    parser.add_argument('--data_path', type=str, 
                       default="", 
                       help='测试数据JSON文件路径')
    parser.add_argument('--diag_item', type=str, default="all", 
                       help='诊断项')
    ...
    parser.add_argument('--save_results', type=str, 
                       default="/home/mzhao/ECHO_VIEW/OutMODEL/Qwen3-VL-8B/result", 
                       help='保存结果到指定JSON文件')
    parser.add_argument('--print_result', action='store_true', default=False, 
                       help='打印结果')
    ...
    for diag_item in diag_items[0:]:
        ...
        with open(f"{args.save_results}/inference_{args.diag_item}.json", 'w', encoding='utf-8') as f:
