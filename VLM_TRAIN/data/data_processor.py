...
def _build_messages(item: Dict[str, Any], base_path: Path) -> List[Dict[str, Any]]:
    ...
    video_pool = [
        ...
        {"type": "video", "video": _make_abs_paths(base_path, vid),"nframes":16,"min_pixels": 224 * 224,
                        "max_pixels": 224 * 224} for vid in videos
    ]
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, processor, data_args,train_or_test="train"):
        super(LazySupervisedDataset, self).__init__()

        if train_or_test=='train':
            dataset = data_args.dataset_use.split(",")
        if train_or_test=='eval':
            dataset = data_args.dataset_use_eval.split(",")
            
        dataset_list = data_list(dataset,data_args.data_json)
        ...
        self.video_min_total_pixels = getattr(
            ...
            # data_args, "video_min_total_pixels", 256 * 256
        )
    ...
    def _get_item(self, sources) -> Dict[str, torch.Tensor]:
        ...
        #     ] * len(video_grid_thw)
        #     print(len(video_grid_thw),len(second_per_grid_ts))
        # else:
        #     video_grid_thw = None
        #     second_per_grid_ts = None
        if "video_grid_thw" in data_dict:
            video_grid_thw = data_dict.get("video_grid_thw")
            # print(f"Original: {video_grid_thw}")
            
            # 确保是 tensor 并去掉可能的 batch 维度
            ...
            elif video_grid_thw.dim() == 1:
                video_grid_thw = video_grid_thw.reshape(1, -1)
            
            # 现在 video_grid_thw 是 (num_videos, 3)
            num_videos = video_grid_thw.size(0)
            
            base_second = self.processor.video_processor.temporal_patch_size / self.processor.video_processor.fps
            second_per_grid_ts = [base_second] * num_videos
            
            # print(f"✓ Processed shape: {video_grid_thw.shape}")
        ...
        position_ids, _ = self.get_rope_index(
            self.merge_size,
            data_dict["input_ids"],
            image_grid_thw=torch.cat(grid_thw, dim=0) if grid_thw is not None else None,
            video_grid_thw=video_grid_thw,  # 直接传入，不要用 if
...
def make_supervised_data_module(processor, data_args) -> Dict:
    ...
    eval_dataset = LazySupervisedDataset(processor, data_args=data_args,train_or_test="eval")
