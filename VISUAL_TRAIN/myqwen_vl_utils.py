...
def smart_nframes(ele, total_frames, video_fps):
    # 从 ele 中获取配置的 nframes，默认为 16
    nframes = ele.get("nframes", 16)
    
    # ========== 新增：自动重复采样逻辑 ==========
    # 如果目标帧数超过视频总帧数，启用重复采样
    if nframes > total_frames:
        # 确保至少保留 2 帧用于采样
        if total_frames < 2:
            raise ValueError(f"Video is too short: only {total_frames} frames, cannot sample.")
        
        # 计算需要重复采样的次数
        repeat_times = (nframes + total_frames - 1) // total_frames  # 向上取整
        
        # 将 nframes 设置为 total_frames，让后续采样通过
        # 但我们需要在 ele 中标记需要重复采样
        ele["_repeat_times"] = repeat_times
        ele["_original_total_frames"] = total_frames
        
        # 临时将 nframes 设为 total_frames，通过校验
        original_nframes = nframes
        nframes = total_frames - 1  # 留1帧余量，确保 <= total_frames
        
        # print(f"[重复采样] 视频共 {total_frames} 帧，目标 {original_nframes} 帧，将重复采样 {repeat_times} 次并拼接")
    # ===========================================
...
def get_video_reader_backend() -> str:
    ...
    # print(f"qwen-vl-utils using {video_reader_backend} to read video.", file=sys.stderr)  # 移除无关噪音
